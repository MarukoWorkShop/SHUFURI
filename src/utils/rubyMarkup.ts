...
/**
 * 修复 AI 未按规范标注、用括号包裹的 inline 注音。
 * 把「港（みなと）」「火(ひ)」转成 {港|みなと}{火|ひ}。
 * 括号明确分隔，不会误伤送假名。
 *
 * 注意：不做裸写修复（汉字后紧跟假名的自动标注），因为日文中「漢字+送假名」
 * 和「漢字+振假名」在文本层面无法区分，自动修复会导致大量误标注。
 * AI 必须严格用 {漢字:かな} 格式标注振假名。
 */
function repairInlineFurigana(text: string): string {
  ...
  // 括号包裹：港（みなと）/港(みなと)/港［みなと］/港《みなと》等
  // 括号明确分隔，可安全覆盖单假名读音（如 火（ひ）），不会误伤送假名
  const repaired = protectedText.replace(
    /([\u4e00-\u9fff々〆ヵヶ]+)[（(［\[《]([ぁ-んァ-ンー\uFF66-\uFF9D]+)[）)\]］》]/g,
    '{$1|$2}',
  );
