...
export function useFillMusicShareFromClipboard({
  ...
  const parseShareText = useCallback(
    async (text: string): Promise<boolean> => {
      ...
      if (!trimmed) {
        showToast(L('请先粘贴音乐软件的分享文案', 'Please paste the music app share text first.'));
        return false;
      }

      // 结构化歌词走另一条 CTA
      if (isStructuredLyricsClipboardText(trimmed) || /(^|\n)[VG]\|/.test(trimmed)) {
        showToast(L('这是歌词内容，请点「粘贴剪贴板歌词」', 'This looks like lyrics. Please tap "Paste Lyrics from Clipboard".'));
        return false;
      }
      ...
      try {
        ...
        if (!musicShare?.title) {
          showToast(L('未识别到 QQ / 网易云分享链接。请复制完整的分享文案后再试', 'Share link not recognized. Please copy the full share text and try again.'));
          return false;
        }
