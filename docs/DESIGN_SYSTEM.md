...
| 角色 | 字体 | 来源 |
|------|------|------|
| 日文歌词 | Kozuka Mincho Pro EL | `KozMinPro-ExtraLight.otf` |
...
| `src/index.css` | 全局重置、安全区、滚动策略 |
| `src/App.css` | 组件样式（待模块化） |
| `src/components/LanguageWheel.css` | 波轮局部样式 |
| `src/utils/furiganaLayout/fonts.ts` | 海报字体常量 |
| `src/utils/furiganaLayout/furiganaPosterShared.ts` | 海报排版 CSS 生成 |
